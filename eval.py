...
DEFAULT_EVAL_DATA_PATH = Path(__file__).resolve().parent / "datasets" / "dev.jsonl"
# 电商场景通过 SEARCH_R1_EVAL_DATA 指向 datasets/ecommerce/dev.jsonl
EVAL_DATA_PATH = Path(os.getenv("SEARCH_R1_EVAL_DATA", str(DEFAULT_EVAL_DATA_PATH)))
...
def trajectory_record(trajectory: Trajectory) -> dict[str, Any]:
    ...
    return {
        ...
        "has_source": trajectory.has_source,
        "source_retrieved": trajectory.source_retrieved,
        "source_gold": trajectory.source_gold,
        "retrieved_doc_ids": trajectory.retrieved_doc_ids,
    ...
    metrics.update(
        {
            ...
            # 引用可溯源（电商场景；英文场景对应字段恒为 False，不影响既有指标）
            "citation/has_source": mean([float(item.has_source) for item in trajectories]),
            "citation/source_retrieved": mean(
                [float(item.source_retrieved) for item in trajectories]
            ),
            "citation/source_gold": mean(
                [float(item.source_gold) for item in trajectories]
            ),
        }
    )
