...
class Trajectory:
    ...
    turns: list[AssistantTurn] = field(default_factory=list)  # 已生成的 assistant 轮数。
    ...
    # 电商场景：轨迹中实际检索到的文档编号（引用真实性校验依据）。
    retrieved_doc_ids: list[str] = field(default_factory=list)
    has_source: bool = False  # 最终答案是否附 Source 行。
    source_retrieved: bool = False  # Source 编号是否来自真实检索结果。
    source_gold: bool = False  # Source 编号是否命中 gold_docs。
...
def finish_search(
    ...
    if result.ok:
        pending.trajectory.retrieved_doc_ids.extend(
            item.doc_id for item in result.items if item.doc_id
        )
    return False
...
def score_trajectory(trajectory: Trajectory) -> None:
    """给完成的轨迹写入格式、精确匹配与引用可溯源奖励。"""
    result = score_answer(
        trajectory.final_text,
        trajectory.example.answers,
        retrieved_doc_ids=trajectory.retrieved_doc_ids,
        gold_doc_ids=trajectory.example.gold_docs or None,
    ...
    trajectory.has_source = result.has_source
    trajectory.source_retrieved = result.source_retrieved
    trajectory.source_gold = result.source_gold
