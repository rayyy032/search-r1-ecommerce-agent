...
SEARCH_BACKENDS = ("deepseek", "wikipedia", "zhihu", "ecommerce")
...
DEFAULT_SEARCH_CONCURRENCY = {"deepseek": 16, "wikipedia": 3, "zhihu": 1, "ecommerce": 8}
DEFAULT_SEARCH_TIMEOUT = {"deepseek": 60.0, "wikipedia": 15.0, "zhihu": 15.0, "ecommerce": 10.0}
...
class SearchItem:
    ...
    url: str | None = None
    doc_id: str | None = None  # 电商知识库的文档编号（引用可溯源奖励用）
...
@dataclass
class EcommerceQdrantClient:
    """检索本地 Qdrant 电商知识库（BGE-small-zh 向量，嵌入式文件存储，免外部服务）。"""

    store_path: str | Path = field(
        default_factory=lambda: Path(__file__).resolve().parent / "ecommerce" / "qdrant_store"
    )
    collection: str = "ecommerce_docs"
    top_k: int = 3
    timeout: float = DEFAULT_SEARCH_TIMEOUT["ecommerce"]  # 保持工厂接口一致；本地检索无网络超时
    stats: SearchStats = field(
        default_factory=lambda: SearchStats(backend="ecommerce")
    )

    def __post_init__(self) -> None:
        base = Path(__file__).resolve().parent
        cache_dir = base.parent / ".cache" / "fastembed"
        cache_dir.mkdir(parents=True, exist_ok=True)
        os.environ.setdefault("FASTEMBED_CACHE_PATH", str(cache_dir))
        # 延迟导入：不使用电商后端时无需加载 onnxruntime
        from fastembed import TextEmbedding
        from qdrant_client import QdrantClient

        path = os.getenv("SEARCH_R1_QDRANT_PATH") or self.store_path
        self._embedder = TextEmbedding(model_name="BAAI/bge-small-zh-v1.5", cache_dir=str(cache_dir))
        self._client = QdrantClient(path=str(path))
        names = {item.name for item in self._client.get_collections().collections}
        if self.collection not in names:
            raise ValueError(
                f"Qdrant 集合 {self.collection} 不存在，请先运行："
                "uv run python 03-search-r1/ecommerce/index_qdrant.py"
            )

    def search(self, query: str) -> SearchResult:
        """语义检索 Top-K 文档，返回带文档编号的证据条目。"""
        started = time.perf_counter()
        with self.stats._lock:
            self.stats.requests += 1
        try:
            qvec = list(self._embedder.query_embed(query))[0]
            hits = self._client.query_points(
                collection_name=self.collection, query=qvec, limit=self.top_k
            ).points
            items = [
                SearchItem(
                    title=str(hit.payload["title"]),
                    content=str(hit.payload["content"])[:800],
                    source="电商商品知识库",
            ...
            with self.stats._lock:
                self.stats.successes += 1
                self.stats.latency_total += latency
                self.stats.result_count += len(items)
            return SearchResult(ok=True, items=items, latency=latency, status=200)
        except Exception as error:  # 本地检索失败也转成工具错误，不中断 rollout
            return self._error_result(started, f"{type(error).__name__}: {error}")

    def _error_result(
        self, started: float, message: str, status: int | None = None
    ) -> SearchResult:
        latency = time.perf_counter() - started
        with self.stats._lock:
            self.stats.errors += 1
...
SearchClient = (
    DeepSeekSearchClient | WikipediaSearchClient | ZhihuSearchClient | EcommerceQdrantClient
)
...
def create_search_client(
    ...
    *,
    ...
    if backend == "ecommerce":
        return EcommerceQdrantClient(timeout=resolved_timeout)
    raise ValueError(f"不支持的搜索后端: {backend}")
...
def format_item(item: SearchItem, index: int) -> str:
    ...
    if item.doc_id:
        lines.insert(1, f"    Doc-ID: {item.doc_id}")
    if item.source:
